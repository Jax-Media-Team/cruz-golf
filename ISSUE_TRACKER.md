...
| MULTI-GROUP-EVENTS | Member-guest tournaments, golf trips, club games (16 players in 4 foursomes) aren't natively supported — each foursome is a disconnected round. Patrick called this out as one of the app's likely defining features. | 🚧 **Phase 1 ready 2026-05-11 (migration 0039 awaiting apply).** Design doc at `docs/MULTI_GROUP_DESIGN.md`. Schema-only Phase 1 adds: `events` table (group_id, name, kind, dates, spectator_token, commissioner_profile_id, soft-delete), `event_games` table (field-wide games), `rounds.event_id` nullable FK. RLS in place; group members can read/write events, spectator token allows public read by ID. TypeScript types shipped in `lib/types.ts` (`GolfEvent`, `EventGame`, `EventKind`). No UI surfaces yet — Phase 2 builds commissioner flow; Phase 3 builds field-wide settlement. Patrick's instincts confirmed: foursome-scoped presses, event-wide skins/Nassau/stroke play, reliability over visuals, simple commissioner controls. |
