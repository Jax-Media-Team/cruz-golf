...
| ARCHIVE-VS-DELETE | Archived rounds were being filtered out of the dashboard's clubhouse-engine fetch (`is("deleted_at", null)`) — so an archived round disappeared from stats / records / clubhouse history too. Patrick's spec: archive = still in history; delete = gone forever. | ✅ **Clarified 2026-05-11.** Removed `deleted_at` filter from the clubhouse-engine query on `/dashboard`. Records / leaderboards / ledger / player stats already filtered by `status='finalized'` (not `deleted_at`), so they were already correct. Active rounds list on `/dashboard` keeps its `deleted_at IS NULL` filter — archived rounds shouldn't appear in active browsing. Clubhouse engine still filters `status='finalized'` internally, so archived non-finalized rounds (e.g. test drafts that got archived) still don't pollute signals. |
| MULTI-GROUP-EVENTS | Member-guest tournaments, golf trips, club games (16 players in 4 foursomes) aren't natively supported — each foursome is a disconnected round. Patrick called this out as one of the app's likely defining features. | 🚧 **Phase 1+2+3a shipped 2026-05-11.** Design doc at `docs/MULTI_GROUP_DESIGN.md`. **Phase 1** (migration 0039, applied): events + event_games tables, rounds.event_id, RLS, spectator-token read. **Phase 2** (UI): `/events/new`, `/events/[id]` event home, AddFoursomeButton, dashboard Events section, round-page "Part of event" chip. **Phase 3a** (engine + UI): `lib/events/settle.ts` with `buildEventFieldStandings()` (aggregated gross/net + thru + foursome status), `settleEventGame()` for field-wide skins / individual stroke, `buildEventBundle()` convenience. `EventLeaderboard` component with Gross/Net/Money/Foursomes tabs + realtime via postgres_changes on every round's scores. Public spectator at `/events/[id]/leaderboard?token=...` — token-gated, no-auth, lists foursomes with per-round watch links. 17 new engine tests. **Phase 3b queued**: event-game picker UI + service-role spectator RPC for full field leaderboard on spectator surface (today it's a foursomes-list hub). Patrick's instincts confirmed: foursome-scoped presses, event-wide skins/Nassau/stroke play, reliability over visuals, simple commissioner controls. |
| GHIN-INTEGRATION-RESEARCH | GHIN/USGA official handicap integration is the biggest long-term feature gap vs competitors. Manual entry today; needs official AGA license to do this compliantly. | 📋 **Research doc shipped 2026-05-11** at `docs/GHIN_RESEARCH.md`. Lays out the landscape: AGA partnership (tier 1, the right answer), manual + refresh prompts (tier 2, what ships today), direct USGA (tier 3). Hard NO on scraping or reverse-engineering the private API. Existing `lib/handicap-provider.ts` is the architectural seam — single-file swap when an AGA license lands. Recommends applying to FSGA's developer program as the first step. Local-override-always-wins safety property is intact + tested. **No autonomous moves here — gated on Patrick's decision per the GHIN/USGA/legal carve-out.** |
| STALE-LIVE-ROUNDS | Admin sees abandoned "live" rounds from yesterday cluttering the view. CLAUDE.md is explicit: no time-driven auto-transitions. | ✅ **Triage surface added 2026-05-11.** New panel on `/admin`: detects rounds with `status='live'` and no score writes in 24h+. Color-coded amber 48h+, red 72h+. Each row deep-links to `/admin/rounds/[id]` where existing finalize / mark-pending / archive controls live. Helper text explains the two triage paths (with scores → finalize or mark pending; without → archive as test draft). No auto-transitions; admin acts manually. |
