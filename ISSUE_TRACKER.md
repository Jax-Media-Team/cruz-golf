...
| MULTI-GROUP-EVENTS | Member-guest tournaments, golf trips, club games (16 players in 4 foursomes) aren't natively supported — each foursome is a disconnected round. Patrick called this out as one of the app's likely defining features. | 📋 **Design doc shipped 2026-05-11** at `docs/MULTI_GROUP_DESIGN.md`. Proposes an additive "Event" concept: optional layer that groups N rounds, supports field-wide games (skins across all 16, net stroke play across the field), shared spectator link, event commissioner. Rounds keep working as today; events are an opt-in container. 4-phase rollout plan. **Awaiting Patrick's review before any schema or code lands.** |
